...
int main(int argc,char *argv[])
{
    ...
    while(1)
    {
        bzero(buffer,sizeof(buffer)); //clean the buffer to avoid any garbage value
        readbytes=read(connfd,buffer,sizeof(buffer)); //read data from the client application
